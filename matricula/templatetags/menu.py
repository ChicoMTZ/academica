...
def print_menu_item(request, menues, is_list=False):
    ...
    for key, menu in items_menu:
        ...
        dev += '<li role="presentation" ><a class="btn btn-success" name="List%d" href="%s"> %s</a>' % (key,ref, ref_display)
        if menu['children']:
...
<style>
    ...
    """
    dev = '<div id="menu" name="menu"><ul class="nav nav-pills" name="List_Menu">' + print_menu_item(context['request'], menues) + "</ul></div>"
